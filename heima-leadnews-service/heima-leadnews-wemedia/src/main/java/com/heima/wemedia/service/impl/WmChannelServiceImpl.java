...
public class WmChannelServiceImpl extends ServiceImpl<WmChannelMapper, WmChannel> implements WmChannelService {
    ...
    public ResponseResult insert(WmChannel adChannel) {
        Integer count= wmChannelMapper.selectCount(Wrappers.<WmChannel>lambdaQuery().eq(WmChannel::getName,adChannel.getName()));
        if(count>0){
            return ResponseResult.errorResult(AppHttpCodeEnum.PARAM_INVALID,"channel name already exists");
        }else{
    ...
    public ResponseResult delete(Integer id) {
        if(wmChannelMapper.selectOne(Wrappers.<WmChannel>lambdaQuery().eq(WmChannel::getId,id))==null){
            return ResponseResult.errorResult(AppHttpCodeEnum.PARAM_INVALID,"channel does not exist");
        }
